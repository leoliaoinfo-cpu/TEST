...
> 最新程式碼在 GitHub：`leoliaoinfo-cpu/TEST` 分支 `claude/caravan-truck-business-system-ul6st3`（最新 commit `95d5021`）。
...
- GitHub：`leoliaoinfo-cpu/TEST`，開發分支 `claude/caravan-truck-business-system-ul6st3`
- **刻意不接 GitHub Pages**：該 repo 的 Pages 網址由使用者另一個系統使用，本分支已從 deploy.yml 移除，**不可覆蓋**。要上線請另開 repo 或子路徑部署
